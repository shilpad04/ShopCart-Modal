function Cart({ cart, onClose, onRemoveFromCart }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
      <div className="relative w-[92vw] max-w-xl max-h-[85vh] overflow-y-auto rounded-2xl border border-slate-800 bg-slate-900 p-5 
      shadow-2xl text-slate-200 font-['Merriweather']">
        <button
          onClick={onClose}
          aria-label="Close cart"
          className="absolute top-3 right-3 inline-flex h-9 w-9 items-center justify-center rounded-full bg-slate-800 
          text-slate-200 hover:bg-slate-700 shadow focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 
            ...
            {cart.map((product) => (
              ...
              >
                ...
                <button
                  ...
                  focus-visible:ring-offset-slate-900 italic"
                >
                  Remove
                </button>
              </div>
